...
; RUN: llvm-profdata merge %t/memprof_pgo.proftext %t/memprof.yaml -o %t.pgomemprofdata
; RUN: llvm-profdata merge %t/memprof_pgo.proftext -o %t.pgoprofdata
; RUN: llvm-profdata merge %t/memprof.nocolinfo.yaml -o %t.nocolinfo.memprofdata

;; Check that the summary can be shown for the indexed profile.
...
; RUN: opt < %t/memprof.ll -passes='memprof-use<profile-filename=%t.memprofdata>' -pgo-warn-missing-function -S -memprof-print-match-info -stats 2>&1 | FileCheck %s --check-prefixes=MEMPROF,ALL,MEMPROFONLY,MEMPROFMATCHINFO,MEMPROFSTATS,AMBIG
...
; RUN: opt < %t/memprof.ll -passes='memprof-use<profile-filename=%t.nocolinfo.memprofdata>' -pgo-warn-missing-function -S 2>&1 | FileCheck %s --check-prefixes=MEMPROFNOCOLINFO,ALL,MEMPROFONLY
...
; RUN: opt < %t/memprof.ll -passes='default<O2>' -memory-profile-file=%t.memprofdata -pgo-warn-missing-function -S 2>&1 | FileCheck %s --check-prefixes=MEMPROF,ALL,MEMPROFONLY,AMBIG

;; Using a pgo+memprof profile for memprof-use should only give memprof metadata
; RUN: opt < %t/memprof.ll -passes='memprof-use<profile-filename=%t.pgomemprofdata>' -pgo-warn-missing-function -S 2>&1 | FileCheck %s --check-prefixes=MEMPROF,ALL,MEMPROFONLY,AMBIG

;; Using a pgo-only profile for memprof-use should give an error
; RUN: not opt < %t/memprof.ll -passes='memprof-use<profile-filename=%t.pgoprofdata>' -S 2>&1 | FileCheck %s --check-prefixes=MEMPROFWITHPGOONLY
; MEMPROFWITHPGOONLY: Not a memory profile

;; Using a memprof-only profile for pgo-instr-use should give an error
; RUN: not opt < %t/memprof.ll -passes=pgo-instr-use -pgo-test-profile-file=%t.memprofdata -S 2>&1 | FileCheck %s --check-prefixes=PGOWITHMEMPROFONLY
...
; RUN: opt < %t/memprof.ll -passes=pgo-instr-use -pgo-test-profile-file=%t.pgomemprofdata -pgo-warn-missing-function -S 2>&1 | FileCheck %s --check-prefixes=PGO,ALL,PGOONLY
...
; RUN: opt < %t/memprof.ll -passes='pgo-instr-use,memprof-use<profile-filename=%t.pgomemprofdata>' -pgo-test-profile-file=%t.pgomemprofdata -pgo-warn-missing-function -S 2>&1 | FileCheck %s --check-prefixes=MEMPROF,ALL,PGO,AMBIG
...
; RUN: opt < %t/memprof.ll -passes='memprof-use<profile-filename=%t.memprofdata>' -pgo-warn-missing-function -S -memprof-report-hinted-sizes -pass-remarks=memory-profile-info 2>&1 | FileCheck %s --check-prefixes=TOTALSIZESSINGLE,TOTALSIZES,TOTALSIZENOKEEPALL,REMARKSINGLESIZE
;; Per-context remarks with size info should be emitted when -memprof-keep-context-size-info is enabled.
; RUN: opt < %t/memprof.ll -passes='memprof-use<profile-filename=%t.memprofdata>' -pgo-warn-missing-function -S -memprof-keep-context-size-info -pass-remarks=memory-profile-info 2>&1 | FileCheck %s --check-prefixes=REMARKSINGLESIZE
;; Only per-allocation remarks should be emitted by default.
; RUN: opt < %t/memprof.ll -passes='memprof-use<profile-filename=%t.memprofdata>' -pgo-warn-missing-function -S -pass-remarks=memory-profile-info 2>&1 | FileCheck %s --check-prefixes=REMARKSINGLEBASIC

;; Check that the total sizes are reported if requested, and prevent pruning
;; via -memprof-keep-all-not-cold-contexts.
; RUN: opt < %t/memprof.ll -passes='memprof-use<profile-filename=%t.memprofdata>' -pgo-warn-missing-function -S -memprof-report-hinted-sizes -memprof-keep-all-not-cold-contexts 2>&1 | FileCheck %s --check-prefixes=TOTALSIZESSINGLE,TOTALSIZES,TOTALSIZESKEEPALL

;; Check that we hint additional allocations with a threshold < 100%.
;; Per-context remarks with size info should be emitted when -memprof-report-hinted-sizes is enabled.
; RUN: opt < %t/memprof.ll -passes='memprof-use<profile-filename=%t.memprofdata>' -pgo-warn-missing-function -S -memprof-report-hinted-sizes -memprof-matching-cold-threshold=60 -pass-remarks=memory-profile-info 2>&1 | FileCheck %s --check-prefixes=TOTALSIZESSINGLE,TOTALSIZESTHRESH60,REMARKSINGLESIZE,REMARKDOMSIZE
;; Per-context remarks with size info should be emitted when -memprof-keep-context-size-info is enabled.
; RUN: opt < %t/memprof.ll -passes='memprof-use<profile-filename=%t.memprofdata>' -pgo-warn-missing-function -S -memprof-matching-cold-threshold=60 -memprof-keep-context-size-info -pass-remarks=memory-profile-info 2>&1 | FileCheck %s --check-prefixes=REMARKSINGLESIZE,REMARKDOMSIZE
;; Only per-allocation remarks should be emitted by default.
; RUN: opt < %t/memprof.ll -passes='memprof-use<profile-filename=%t.memprofdata>' -pgo-warn-missing-function -S -memprof-matching-cold-threshold=60 -pass-remarks=memory-profile-info 2>&1 | FileCheck %s --check-prefixes=REMARKSINGLEBASIC,REMARKDOMBASIC
...
; RUN: opt < %t/memprof.ll -passes='memprof-use<profile-filename=%t.memprofdata>' -pgo-warn-missing-function -S -memprof-cloning-cold-threshold=80 -memprof-keep-all-not-cold-contexts 2>&1 | FileCheck %s --check-prefixes=TOTALSIZES,TOTALSIZESKEEPALL

;; Make sure we emit a random hotness seed if requested.
; RUN: llvm-profdata merge -memprof-random-hotness %t/memprof.yaml -o %t.memprofdatarand 2>&1 | FileCheck %s --check-prefix=RAND
...
; RUN: opt < %t/memprof.ll -passes='memprof-use<profile-filename=%t.memprofdatarand>' -pgo-warn-missing-function -S -stats 2>&1 | FileCheck %s --check-prefixes=ALL,MEMPROFONLY,MEMPROFSTATS

;; Make sure we use a specific random hotness seed if requested.
; RUN: llvm-profdata merge -memprof-random-hotness -memprof-random-hotness-seed=1730170724 %t/memprof.yaml -o %t.memprofdatarand2 2>&1 | FileCheck %s --check-prefix=RAND2
; RAND2: random hotness seed = 1730170724
; RUN: opt < %t/memprof.ll -passes='memprof-use<profile-filename=%t.memprofdatarand2>' -pgo-warn-missing-function -S -stats 2>&1 | FileCheck %s --check-prefixes=MEMPROFRAND2,ALL,MEMPROFONLY,MEMPROFSTATS
...
; RUN: opt < %t/memprof.ll -passes='memprof-use<profile-filename=%t.memprofdata>' -pgo-warn-missing-function -S -memprof-print-match-info -stats -memprof-min-ave-lifetime-access-density-hot-threshold=0 -memprof-use-hot-hints 2>&1 | FileCheck %s --check-prefixes=MEMPROFHOT,ALL

;; However, with the same threshold, but hot hints not enabled, it should be
;; notcold again.
; RUN: opt < %t/memprof.ll -passes='memprof-use<profile-filename=%t.memprofdata>' -pgo-warn-missing-function -S -memprof-min-ave-lifetime-access-density-hot-threshold=0 2>&1 | FileCheck %s --check-prefixes=MEMPROF,ALL,AMBIG

;; Test that we don't get an ambiguous memprof attribute when
;; -memprof-ambiguous-attributes is disabled.
; RUN: opt < %t/memprof.ll -passes='memprof-use<profile-filename=%t.memprofdata>' -pgo-warn-missing-function -S -memprof-ambiguous-attributes=false 2>&1 | FileCheck %s --check-prefixes=MEMPROF,ALL,NOAMBIG
...
clang++ $COMMON_FLAGS -fmemory-profile memprof.cc -o memprof.exe
env MEMPROF_OPTIONS=log_path=stdout ./memprof.exe > memprof.memprofraw
llvm-profdata merge memprof.memprofraw --profiled-binary memprof.exe -o memprof.profdata
llvm-profdata show --memory memprof.profdata
echo ""
echo ";--- memprof.nocolinfo.yaml"
clang++ $COMMON_FLAGS -gno-column-info -fmemory-profile memprof.cc -o memprof.nocolinfo.exe
env MEMPROF_OPTIONS=log_path=stdout ./memprof.nocolinfo.exe > memprof.nocolinfo.memprofraw
llvm-profdata merge memprof.nocolinfo.memprofraw --profiled-binary memprof.nocolinfo.exe -o memprof.nocolinfo.profdata
llvm-profdata show --memory memprof.nocolinfo.profdata
echo ""
echo ";--- memprof_pgo.proftext"
clang++ $COMMON_FLAGS -fprofile-generate=. memprof.cc -o pgo.exe
env LLVM_PROFILE_FILE=memprof_pgo.profraw ./pgo.exe
llvm-profdata merge --text memprof_pgo.profraw -o -
;--- memprof.yaml
---
# MemProfSummary:
#   Total contexts: 8
#   Total cold contexts: 5
#   Total hot contexts: 0
#   Maximum cold context total size: 10
...
HeapProfileRecords:
  - GUID:            0xd29e2fe34de9ae40
    CallSites:
      - Frames:
          - { Function: 0xd29e2fe34de9ae40, LineOffset: 1, Column: 0, IsInlineFrame: false }
  - GUID:            0xf129122801e64264
    CallSites:
      - Frames:
          - { Function: 0xf129122801e64264, LineOffset: 1, Column: 0, IsInlineFrame: false }
  - GUID:            0xdb956436e78dd5fa
    AllocSites:
      - Callstack:
          - { Function: 0xdb956436e78dd5fa, LineOffset: 4, Column: 0, IsInlineFrame: false }
        MemInfoBlock:
          AllocCount:      1
          TotalSize:       10
          TotalLifetime:   0
          TotalLifetimeAccessDensity: 20000
      - Callstack:
          - { Function: 0xdb956436e78dd5fa, LineOffset: 5, Column: 0, IsInlineFrame: false }
        MemInfoBlock:
          AllocCount:      1
          TotalSize:       10
          TotalLifetime:   200000
    ...
    CallSites:
      - Frames:
          - { Function: 0xdb956436e78dd5fa, LineOffset: 6, Column: 0, IsInlineFrame: false }
      - Frames:
          - { Function: 0xdb956436e78dd5fa, LineOffset: 7, Column: 0, IsInlineFrame: false }
      - Frames:
          - { Function: 0xdb956436e78dd5fa, LineOffset: 8, Column: 0, IsInlineFrame: false }
      - Frames:
          - { Function: 0xdb956436e78dd5fa, LineOffset: 9, Column: 0, IsInlineFrame: false }
      - Frames:
          - { Function: 0xdb956436e78dd5fa, LineOffset: 31, Column: 0, IsInlineFrame: false }
  - GUID:            0x251b60c9a8c4382a
    CallSites:
      - Frames:
          - { Function: 0x251b60c9a8c4382a, LineOffset: 2, Column: 0, IsInlineFrame: false }
      - Frames:
          - { Function: 0x251b60c9a8c4382a, LineOffset: 3, Column: 0, IsInlineFrame: false }
  - GUID:            0x7f8d88fcc70a347b
    AllocSites:
      - Callstack:
          - { Function: 0x7f8d88fcc70a347b, LineOffset: 1, Column: 0, IsInlineFrame: false }
          - { Function: 0xdb956436e78dd5fa, LineOffset: 6, Column: 0, IsInlineFrame: false }
        MemInfoBlock:
          AllocCount:      1
          TotalSize:       10
      ...
      - Callstack:
          - { Function: 0x7f8d88fcc70a347b, LineOffset: 1, Column: 0, IsInlineFrame: false }
          - { Function: 0xbfa73ad9b09bd18c, LineOffset: 1, Column: 0, IsInlineFrame: false }
          - { Function: 0xf129122801e64264, LineOffset: 1, Column: 0, IsInlineFrame: false }
          - { Function: 0xdb956436e78dd5fa, LineOffset: 8, Column: 0, IsInlineFrame: false }
        MemInfoBlock:
          AllocCount:      1
