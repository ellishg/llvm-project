...
TEST_F(InstrProfTest, test_merge_traces_truncated) {
  uint64_t TraceReservoirSize = 10;
  ...
  Writer.addFunctionTraces({LargeTrace, SmallTrace}, 2);

  auto Profile = Writer.writeBuffer();
  readProfile(std::move(Profile));

  ASSERT_TRUE(Reader->isTemporalProfile());
  EXPECT_EQ(Reader->getTraceStreamSize(), 2);
  EXPECT_THAT(Reader->getFunctionTraces(),
...
TEST_F(InstrProfTest, test_merge_traces_from_writer) {
  ...
  InstrProfWriter Writer(false, TraceReservoirSize, MaxTraceLength);
  InstrProfWriter Writer2(false, TraceReservoirSize, MaxTraceLength);
  ASSERT_THAT_ERROR(Writer.mergeProfileKind(InstrProfKind::TemporalProfile),
                    Succeeded());
  ...
  Writer.addFunctionTraces({FooTrace}, 1);
  Writer2.addFunctionTraces({BarTrace}, 1);
  Writer.mergeRecordsFromWriter(std::move(Writer2), Err);
...
TEST_F(InstrProfTest, test_merge_traces_sampled) {
  ...
  Writer.addFunctionTraces({FooTrace, BarTrace, GooTrace}, 5);
  // Add some unsampled traces
  Writer.addFunctionTraces({BarTrace, GooTrace}, 2);
  Writer.addFunctionTraces({FooTrace}, 1);
  ...
  ASSERT_TRUE(Reader->isTemporalProfile());
  EXPECT_EQ(Reader->getTraceStreamSize(), 8);
  // Check that we have a subset of all the traces we added
  EXPECT_THAT(Reader->getFunctionTraces(), SizeIs(TraceReservoirSize));
  EXPECT_THAT(
      Reader->getFunctionTraces(),
